...
export function LoginForm({ lang }: { lang: Lang }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (res?.ok) {
      // open-redirect koruması: yalnız '/' ile başlayan yerel yollar ('//' = protokolsüz dış URL, yasak)
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div>
        <label htmlFor="login-email" className="mb-1.5 block text-[11px] font-data uppercase tracking-wider text-muted">
      ...
      <div>
        <label htmlFor="login-password" className="mb-1.5 block text-[11px] font-data uppercase tracking-wider text-muted">
